#!/usr/bin/env python3
"""
build-report.py - Reporte HTML de calidad.

Lee un verification.json y produce report.html al lado. Aplica la misma regla
del gate para que el reporte y quality-gate.py no se contradigan.
"""
import json
import os
import sys
...
OK, BAD = "#2E7D52", "#B3402F"
INK, PAPER = "#0E1A26", "#F3F4F1"
BRAND, GOLD = "#9E1B2E", "#C99A3A"
...
<style>
 ...
 .verdict{{display:inline-block;background:{vcolor};color:#fff;font:800 20px/1 IBM Plex Mono,monospace;padding:12px 22px;border-radius:8px;margin:14px 0 28px}}
 .pillars{{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-bottom:28px}}
 .card{{border:1px solid #d7dad5;border-radius:8px;padding:16px;background:#fbfcfa}}
 ...
 h2{{font-size:13px;letter-spacing:.1em;text-transform:uppercase;color:#8F6A1C;margin:24px 0 6px}}
 ...
 <div class="pillars">
   ...
   <div class="card"><h3>Seguridad {chip(seg_ok)}</h3>
     <div class="big">{s.get('critical',0)} criticas</div>
 ...
 <table><thead><tr><th>Severidad</th><th>Ubicacion</th><th>Nota</th></tr></thead><tbody>{filas_seg}</tbody></table>
